...
class CameraInfo(NamedTuple):#CameraInfo 类保存了相机的位姿（旋转矩阵和平移向量）、视场角、图像数据和其他相关信息。
    uid: int
    R: np.array
    T: np.array
    FovY: np.array
...
class SceneInfo(NamedTuple):#SceneInfo 类保存了场景的点云数据、训练和测试的相机列表、用于归一化的NeRF数据以及点云文件的路径
...
def getNerfppNorm(cam_info):#计算相机中心的平均值和场景的对角线长度，用于归一化NeRF数据
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):#读取并处理COLMAP生成的相机外参和内参数据，生成CameraInfo对象列表。
    cam_infos = []
...
def fetchPly(path):#读取PLY文件中的点云数据并返回BasicPointCloud对象
...
def storePly(path, xyz, rgb):#将点云数据存储到PLY文件中。
...
def readColmapSceneInfo(path, images, eval, lod, llffhold=8):#读取COLMAP生成的场景信息，包括相机参数和点云数据，并返回SceneInfo对象。
    try:
